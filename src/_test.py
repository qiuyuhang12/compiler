...
for testcase in test_file:
    try:
        ...
        try:
            _, _ = process.communicate(input=content, timeout=2)
            compile_status = process.returncode
            if compile_status != 0:
                raise Exception("LLVM Compile Error")
        except subprocess.TimeoutExpired:
            process.terminate()
            print(testcase, red_msg.format(msg="timeout"))
            continue
        ...
        process = subprocess.Popen(commands, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                   text=True, shell=True)
        try:
            stdout, _ = process.communicate(input="", timeout=1)
            compile_status = process.returncode
            if compile_status != 0:
        ...
        process = subprocess.Popen(commands, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                   text=True, shell=True)
        try:
            stdout, _ = process.communicate(input=input_data, timeout=2)
